...
- **Hero 左上的字標要與分享卡一致**：`和聖壇`（直式 h1）＋短線＋`台北古亭`，跟 `scripts/assets/og-hero-wordmark.jpg` 同一個構成。分享卡上一直有地名、網站上原本沒有，廟方 2026-09-12 指出後補上。地名不放進 h1（h1 是宮壇名，分頁標題與 og:title 已帶「台北古亭」）；它落在上緣遮罩之外、白字壓亮金，靠 `text-shadow` 貼著筆畫的暗影撐可讀性，844×390 這種矮橫屏會落進底部淡出區，實測仍讀得到。
- **分享卡的網址也帶版號**（2026-09-12）：`og-hero.jpg` 列在 `HERO_FILES` 裡，index.html 的 og:image／twitter:image／JSON-LD 建置時都變成 `og-hero.jpg?v=<雜湊>`。LINE／FB 拿圖的網址當快取鍵，換圖不換網址永遠是舊卡。但對方另外還快取「這個網頁的 og 標籤」：已分享過的網址要等它重抓——FB 用 Sharing Debugger 按重新抓取；LINE 沒有工具，分享時網址後面加 `?v=2` 之類就是新網址。
...
- **Hero 三尊的身分弄錯過三次（`HERO_DEITIES`）**：左前＝濟公、中後＝**天上聖母三媽（主神、橘袍黑面）**、右前＝**天上聖母二媽（黃袍金冠）**。
  ...
  **已知資料衝突，動身分之前一定要問廟方**：舊那批 `神明正照/二媽.jpg` 是「紅袍藍龍紋」那尊、不是黃袍，新舊兩批對「二媽」的指認不一致。現行採用新那批＋廟方 2026-09-02 的指認。
  ...
  換照片時 `drop` 一定要重算：`臉高 = 圖高×(1−臉在圖中的相對位置) − drop`。現行值：濟公 0.25、三媽 0.28、二媽 0.35。臉的位置用「每 5% 一條網格」目視讀，別寫顏色規則自動找——三媽是黑面，她的手與軀幹跟臉同色，程式會抓到胸口。
  **濟公 2026-09-12 換成坐姿持扇那張**（廟方指定；原圖 `~/Downloads/DSC09730.png`，灰背板，走 `build-hero-assets.cjs` 去背）。臉的位置 0.25 與舊的 0.24 只差 0.57vh，`drop` 沿用；但**寬高比 1045:1600 比舊的 789:1600 寬 32%**，手機那排本來剛好貼齊兩邊，多出的 11.8vw 全數吃進濟公與三媽的重疊（三媽手機 `gap` 24vw→36vw），桌機那排置中不用改。`og-hero.jpg` 已重量重產（只有濟公的 left/w 變）。
- **Hero 構圖是「主神在後、二媽與濟公在她面前」**（廟方 2026-09-02 給合成圖指定）：三媽最高最大但 `z-1` 疊最底層，身體被前面兩尊擋住、只露出頭與冠帽；前兩尊的臉大致齊高。高度比 57:98:76（濟公:三媽:二媽）是從那張合成圖量出來的。
  ...
  **驗收數字換濟公照片後重測**（2026-09-12）：1280×800 是 0.00/0.43/0.00%、375×812 是 0.00/0.31/0.00%（那零點幾仍是二媽冠帽尖擦到三媽頭部邊緣）。濟公的帽頂在每個尺寸都剛好落在三媽肩線之下（差 5–12px），再把他往上抬就會碰頭。
  `scripts/build-hero-assets.cjs` **只有濟公是它產的**（2026-09-12 起）；三媽、二媽是廟方 2026-09-01～02 直接給的已去背原圖，只裁 alpha 邊界＋縮到 1600 高。**跑的時候一定帶第二個參數指定哪一尊**（`… /tmp/hero-out hero-jigong`），不帶會把三尊全跑一遍、拿舊照片蓋掉線上的圖。成品統一 1600 高，畫布用 `canvas` 放大到夠、裁完再縮。底座下方那條檯面與影子（s≈0.2，pred 切不掉）由 `trimFloor` 事後砍，不要為它放寬 pred——扇子的銀絲會跟著沒。
...
- 匯款資訊有**兩個不同的帳戶，不可混用**：
  ...
  **遷址募資專款**原本是第一銀行 007 古亭分行 171-68-143732 王順文，**2026-09-12 起從 `/relocation` 撤下**（廟方：等協會成立、專款帳戶設好再提供正式匯款資訊），頁尾改放廟方給的說明文案（`RemittanceNotice`）。之後的專款帳戶不一定是同一個，放回去前要跟廟方重新確認，別直接拿舊的。
...
- **揪團已上線**（2026-09-09，普渡報名結束後合併 `feature/group-booking` 的 a4fd261）。
  ...
  **主揪可以主動刪除**（2026-09-12 廟方要求，原本只能等 7 天到期）：面板最下方一行淡色文字「刪除這張揪團報名表」、提示卡每一列一個垃圾桶圖示。刻意做小做淡、跟「加入」「送出」拉開——長輩的手指按不準，破壞性動作不能挨著主要動作。`confirm()` 警示寫明兩件無法復原的事：名單一起刪、分享出去的連結會失效。RLS `owner_delete_shared_sessions` 只放行 `created_by = auth.uid()`；**舊場次（created_by NULL）刪不掉是刻意的**，放寬會讓任何拿到連結的人能刪別人的表，它們最多 7 天就過期。**`delete()` 被 RLS 擋下是靜默 0 列不會報錯**，要 `.select('id')` 讀回實際刪掉的列數，否則會告訴主揪「已刪除」但東西還在。
  **會員中心看得到揪團紀錄**（2026-09-12 廟方：「紀錄很重要」）：報名紀錄 tab 最上面一段「揪團報名表」，列出這個帳號開過的每一張表——進行中（繼續／刪除）、已送出（只能看，紀錄要留）、已過期沒送出（那一團沒成，主揪要知道）。資料走 `get_my_shared_history()`（`shared_sessions_history.sql`），與 `get_my_shared_sessions()` 刻意分兩支：前者回全部，後者只回還能動的給提示卡用。**為什麼不能靠既有的報名紀錄**：那是拿電話比對 `lamp_registrations` 等表撈的，揪團送出時親友填了自己電話的那幾筆掛在親友名下，主揪看不到，同一團會缺人；揪團紀錄直接列場次＋名單才是完整的一團。已送出的場次名單（`shared_session_entries`）保留不刪，就是為了這裡。
  **親友點到失效連結要有提示**：`?share=` 指到不存在的表（被刪或到期）原本靜靜 return，親友看到一般頁面會以為網站壞了。現在三個服務頁最上方顯示「找不到這張揪團報名表」並保留一般登記表讓他自行登記。
